...
import HistoryMaintenanceTable from "./historytable";
import RecordMaintenanceForm from "./recordmaintenance";
...
const History = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${baseURL}/maintenances/history/${org_id}/${user_id}/`)
      ...
      .then((data) => {
        setHistoryMaintenance(data.maintenance_list || []); // Ensure we set an empty array if maintenance_list is undefined
        setLoading(false);
      })
      .catch((error) => {
