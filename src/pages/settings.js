...
const Settings = () => {
	...
	return (
		<div>
			<div style={{ display: 'flex', fontWeight: 'bold', fontSize: '1em' }}>Settings</div>
			<div style={{ display: 'grid', border: '1px solid gray', margin: '2em', overflow: 'scroll' }}>
				<div style={{ display: 'flex' }}>
					<button onClick={() => handleTabClick('General')} style={tabButtonStyle(activeTab === 'General')}>
						General
					</button>
					<button onClick={() => handleTabClick('Users')} style={tabButtonStyle(activeTab === 'Users')}>
						Users
					</button>
					<button onClick={() => handleTabClick('Billing')} style={tabButtonStyle(activeTab === 'Billing')}>
						Billing
					</button>
					<button onClick={() => handleTabClick('Notification')} style={tabButtonStyle(activeTab === 'Notification')}>
						Notification
					</button>
					<button onClick={() => handleTabClick('Security')} style={tabButtonStyle(activeTab === 'Security')}>
						Security
					</button>
				</div>
